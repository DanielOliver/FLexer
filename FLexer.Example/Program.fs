...
let rules: Rule<LexerMode, TokenType> array = 
  [  Rule.From LexerMode.Normal TokenType.Whitespace.NoFun None "Whitespace" (Regex("[^\S\r\n]+"))
     ...
     Rule.From LexerMode.Normal (fun t -> t.Substring(0, t.Length-1) |> Convert.ToDecimal |> TokenType.Decimal) None "Decimal" (Regex "[0-9]+[.][0-9]+m")
     ...
     Rule.From LexerMode.Normal TokenType.StringDelimiter (Some(RuleAction.PushMode(LexerMode.String))) "StringLiteralDelimiterIn" (Regex("[\"]{3}"))
     Rule.From LexerMode.String TokenType.StringLiteral None "StringLiteral" (Regex("(.|\s)+?(?=[\"]{3})"))
     Rule.From LexerMode.String TokenType.StringDelimiter (Some(RuleAction.PopModeBefore)) "StringLiteralDelimiterOut" (Regex("[\"]{3}"))
